...
for (int i=0; i<joint_names.size(); i++) 
   ...
   std::fstream values;
   values.open ("/home/brad/Desktop/Tests/pacer-tests/BotBuilder/values.txt", std::ios::in | std::ios::out | std::ios::ate);
...
for(int joint=0;joint<joint_names.size(); joint++)
{
		...
		qdd[joint_names[joint]][0] = allqddVals[currLine][joint];
	        ...
	        values << "\n" << "Position: " << allqVals[currLine][joint] << "\n";
		values << "\n" << "Velocity: "<< allqdVals[currLine][joint] << "\n";
		values << "\n" << "Acceleration: " << allqddVals[currLine][joint] << "\n";
}
