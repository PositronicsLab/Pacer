...
int moby_main(int argc, char** argv)
{
    ...
    if (argc < 1)
    {
        std::cerr << "syntax: driver [OPTIONS]" << std::endl;
    ...
    if (std::string(MODEL_FILE).find(".xml") != std::string::npos)
        READ_MAP = XMLReader::read(std::string(MODEL_FILE));
    else if (std::string(MODEL_FILE).find(".sdf") != std::string::npos)
    {
        // artificially create the read map
        shared_ptr<EventDrivenSimulator> eds = SDFReader::read(std::string(MODEL_FILE));
        READ_MAP[eds->id] = eds;
    ...
    if (!s)
    {
        std::cerr << "driver: no simulator found in " << MODEL_FILE << std::endl;
        return -1;
    }
    ...
    if (std::string(MODEL_FILE).find(".xml") != std::string::npos)
        process_xml_options(std::string(MODEL_FILE));
...
int main(int argc, char** argv){
    std::cerr << "syntax: driver <xml model file> <xml vars file> [OPTIONS] --simulator [SIMULATOR OPTIONS]" << std::endl;
    ...
    MODEL_FILE = std::string(&argv[1][0]);
    VARS_FILE  = std::string(&argv[2][0]);
    ...
    for (int i=3; i< argc-1; i++)
    {
        ...
        else if (option.find("-aa=") != std::string::npos)
        {
//            MAX_TIME = std::atof(&argv[i][TWOCHAR_ARG]);
//            MAX_ITER = std::atoi(&argv[i][TWOCHAR_ARG]);
        }
        else if (option.find("-a=") != std::string::npos)
        {
//            scene_path = std::string(&argv[i][ONECHAR_ARG]);
//            strcpy(THREED_EXT, &argv[i][ONECHAR_ARG]);
        }
        ...
        {
        } else if (option.find("--moby-driver") != std::string::npos)
            moby_main(argc-i,&argv[i]);
    }
}
