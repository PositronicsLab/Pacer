...
void Update(const boost::shared_ptr<Pacer::Controller>& ctrl, double t){
  ...
  for(int i=0;i<foot_names.size();i++){
    Ravelin::Origin3d x = ctrl->get_data<Ravelin::Origin3d>(foot_names[i]+".init.x"),
      xd(0,0,0),
      xdd(0,0,0);
    ctrl->set_data<Ravelin::Origin3d>(foot_names[i]+".goal.x",x);
    ctrl->set_data<Ravelin::Origin3d>(foot_names[i]+".goal.xd",xd);
    ctrl->set_data<Ravelin::Origin3d>(foot_names[i]+".goal.xdd",xdd);
