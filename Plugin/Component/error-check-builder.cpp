...
else
{
	...
	int lines=0;
	//see if there are enough models to do bifurcation testing
        while (std::getline(matlab, fileLine) && lines<3)
	{
           ++lines;
	}
	//if there are enough, then run those tests
	//currently just a stand-in that goes straight to the editor
	//Sam's code will be linked to here
        if(lines>2)
	{
	...
	//otherwise, go straight to the editor
	else
	{
