...
int main( int argc, char* argv[] ) {
  ...
  for (int i=0;i<N_THREADS; i++) {
    static int iret = pthread_create( &worker_threads[i], NULL,&thread_worker, (void *)i);
    if(iret)
    {
  ...
  while (true) {
    ...
    int product = std::accumulate(thread_complete.begin(), thread_complete.end(), 1, std::multiplies<int>());
